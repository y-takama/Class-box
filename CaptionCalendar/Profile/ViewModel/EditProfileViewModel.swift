...
//                    self.user.backImageUrl = backImageUrl!
    ...
    func saveUserInfo(fullname: String, username: String, instagramText: String, twitterText: String) {
        guard let uid = user.id else { return }
        let data = ["fullname": fullname,
                    "username": username,
                    "instagramText": instagramText,
                    "twitterText": twitterText] as [String: Any]
        COLLECTION_USERS.document(uid).updateData(data) { _ in
//            self.user.fullname = fullname
//            self.user.username! = username
//            self.user.instagramText! = instagramText
//            self.user.twitterText! = twitterText
            AuthViewModel.shared.fetchUserSetting()
