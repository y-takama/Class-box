    ...
    var body: some View {
        NavigationView {
            VStack(alignment: .leading, spacing: 15) {
                ForEach(viewModels.textbookName, id: \.self) { info in
                    NavigationLink(destination: AdminStudyView(viewModel: StudyContentViewModel(content: info), cource: info, user: user), label: {
                        Text(info.textbookName!)
                            .font(.system(size: 15))
                            .bold()
                    })
                }
