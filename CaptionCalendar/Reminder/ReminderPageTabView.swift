...
struct ReminderPageView<Content: View, Label: View>: View {
    var content: Content
    var label: Label
    var tint: Color
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Capsule()
                .fill(tint)
                .frame(width: maxTabs == 0 ? 0 : ((getScreenBounds().width*3/5)/maxTabs), height: 2)
                .padding(.top, 8)
                .frame(maxWidth: .infinity, alignment: .leading)
                .offset(x: tabOffset)
            ReminderOffSetPageTabView(selection: $selection, offset: $offset) {
                HStack(spacing: 0) {
                    content
                }
                .overlay(
                ...
                .onPreferenceChange(TabPreferenceKey.self) { proxy in
                    ...
                    let screenWidth = getScreenBounds().width*3/5
