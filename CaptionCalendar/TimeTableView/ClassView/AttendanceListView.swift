...
//                            .padding()
    ...
    func rowRemove(offsets: IndexSet, listId: String, classId: String , status: String) {
        ...
        let dogRef = COLLECTION_USERS.document(uid).collection("2021LH").document(classId).collection("AttendanceList").document(listId)
        dogRef.delete() { err in
            if let err = err {
                print("Error removing document: \(err)")
            }
            if status == "出席" {
                COLLECTION_USERS.document(uid).collection("2021LH").document(classId).updateData(["attendance": viewModel.classinfo.attendance! - 1]) { _ in
                }
            }
            if status == "遅刻" {
                COLLECTION_USERS.document(uid).collection("2021LH").document(classId).updateData(["behindtime": viewModel.classinfo.behindtime! - 1]) { _ in
                }
            }
            if status == "欠席" {
                COLLECTION_USERS.document(uid).collection("2021LH").document(classId).updateData(["absence": viewModel.classinfo.absence! - 1]) { _ in
                }
            }
        }
    }
