    ...
    func removeClass() {
        ...
        dogRef.delete() { err in
            if let err = err {
                ...
                COLLECTION_USERS.document(uid).collection("2021LH").document(classId).updateData(["attendance": attendance - 1]) { _ in
                    mode.wrappedValue.dismiss()
                }
            }
            else if classes.status == "遅刻" {
                let behindtime = viewModel.classInfo.behindtime!
                COLLECTION_USERS.document(uid).collection("2021LH").document(classId).updateData(["behindtime": behindtime - 1]) { _ in
                    mode.wrappedValue.dismiss()
                }
            }
            else if classes.status == "欠席" {
                let absence = viewModel.classInfo.absence!
                COLLECTION_USERS.document(uid).collection("2021LH").document(classId).updateData(["absence": absence - 1]) { _ in
                    mode.wrappedValue.dismiss()
                }
            }
        }
